...
def Message processData(Message message) {
    ...
    def gitlabAccessTokenAlias = prop.get("GITLAB_ACCESS_TOKEN")
    if(gitlabAccessTokenAlias == null || gitlabAccessTokenAlias.trim().length() == 0 || gitlabAccessTokenAlias.trim() == '""')
        throw new Exception("\'Error: Name of the credential created in Security Material to store Gitlab Personal Access Token is missing.\'")

    def gitlabAccessToken = getGitlabToken(gitlabAccessTokenAlias)
    message.setProperty("GITLAB_ACCESS_TOKEN", gitlabAccessToken)

    def gitlabCommitMsg = prop.get("GITLAB_COMMIT_MESSAGE")
    if(gitlabCommitMsg == null || gitlabCommitMsg.trim().length() == 0 || gitlabCommitMsg.trim() == '""')
        throw new Exception("\'Error: GitLab Commit message is missing\'")

    def runMode = prop.get("RUN_MODE")
    if(runMode == null || runMode.trim().length() == 0 || runMode.trim() == '""')
        throw new Exception("\'Error: Value for Select either Create or Update file contents in GitLab is missing. Allowed values: create,update.\'")
    if(runMode && runMode.toString().trim().length() != 0) {
        def validValues = ["CREATE", "UPDATE"]
        if (!validValues.contains(runMode.trim().toUpperCase()))
            throw new Exception("\'Error: Value for Select either Create or Update file contents in GitLab is not valid. Allowed values: create,update.\'")
    }

    def uploadFileType = prop.get("UPLOAD_FILE_TYPE")
    if(uploadFileType == null || uploadFileType.trim().length() == 0 || uploadFileType.trim() == '""')
        throw new Exception("\'Error: Value for Upload to GitLab as a \'ZIP\' or an \'UNZIP\' File is missing. Allowed values: zip,unzip.\'")
    if(uploadFileType && uploadFileType.toString().trim().length() != 0){
        ...
        if (!validValues.contains(uploadFileType.trim().toUpperCase()))
            throw new Exception("\'Error: Value for Upload to GitLab as a \'ZIP\' or an \'UNZIP\' File is not valid. Allowed values: zip,unzip.\'")
    ...
    if(integrationContentType == null || integrationContentType.trim().length() == 0 || integrationContentType.trim() == '""')
        throw new Exception("\'Error: Select Artifact Type to Sync to GitLab is missing. Allowed values: all,pkg,ifl,vmp.\'")
    if(integrationContentType && integrationContentType.toString().trim().length() != 0){
        ...
        inputValues.each { value ->
            ...
            if (!validIntContentType.contains(trimmedValue))
                throw new Exception("\'Error: Value for Select Artifact Type to Sync to GitLab is not valid. Allowed values: all,pkg,ifl,vmp.\'")
        }
    }
    ...
    if(integrationFlowId && integrationFlowId.toString().trim().length() != 0){
        ...
        if (!inputValues.contains('ALL') && !inputValues.contains('IFL')) {
            throw new Exception("\'Error: If Integration Flow Id's exists, then Value for Select Artifact Type to Sync to GitLab must either contain all or ifl\'")
        }
    }
    ...
    if(valueMappingId && valueMappingId.toString().trim().length() != 0){
        ...
        if (!inputValues.contains('ALL') && !inputValues.contains('VMP')) {
            throw new Exception("\'Error: If Value Mapping Id's exists, then Value for Select Artifact Type to Sync to GitLab must be either contain all or vmp\'")
        }
    }
...
String getGitlabToken(String keyAlias)
{
    ...
    try
    {
        ...
        throw new Exception("\'Error: Secure Parameter for GitLab Personal Access Token is not available\'")
    }
}
